...
- 詳細画面の「Google の評価・クチコミ」は任意機能。Google Maps の API キー
  （Places API (New) を有効化したもの）を設定すると、評価・件数・営業状況・
  クチコミを表示し、「開く」ボタンが Google 側の正確な場所ページに切り替わる。
  キーは端末の localStorage にだけ保存され、Google 以外には送信されない
- 0件に近いときは半径を自動で3倍（上限 50km）に広げて一度だけ再検索する。
  逆に密集地で件数が上限（300）に達したときは半径を半分に絞って取り直す
  （Overpass の件数打ち切りは距離順ではないため、最寄りの欠落を防ぐ）
- 結果は同一地点(約100m格子)×カテゴリーで10分間キャッシュする
- 位置情報は端末内でのみ使用し、検索座標が Overpass API（キー設定時は Google）
  に送られる以外はどこにも保存・送信されない
...
- Google クチコミ（任意）: [Places API (New)](https://developers.google.com/maps/documentation/places/web-service/op-overview)
  の Text Search。API キー設定時のみ動き、これだけは無料枠あり・キー必要
